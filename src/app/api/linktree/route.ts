      ...
      if (links.length > 0) {
        await prisma.linktreeLink.createMany({
          data: links.map((link: any, idx: number) => ({
            ...
            customIconScale: typeof link.customIconScale === 'number' && !isNaN(link.customIconScale) ? Math.max(30, Math.min(Math.round(link.customIconScale), 250)) : 100,
            customIconShape: typeof link.customIconShape === 'string' ? link.customIconShape : 'auto',
            customIconFit: typeof link.customIconFit === 'string' ? link.customIconFit : 'contain',
